...
class TestModemInputFilesBuilder(TestCase):
    ...
    def test_custom_initial_res(self):
        edipath = EDI_DATA_DIR  # path where edi files are located
        # set the dir to the output from the previously correct run
        self._expected_output_dir = os.path.join(SAMPLE_DIR, 'ModEM', 'custom_res')
        ...
        # assert failure on bad depth ranges
        with pytest.raises(ValueError):
            mo = Model(station_locations=do.station_locations,
                       ...
                       pad_method='stretch',
                       z_target_depth=120000,
                       res_initial_method='by_range',
                       res_initial_custom_values=[200],
                       res_initial_custom_depth_ranges=[(1000, 0)])
            mo.make_mesh()


        # assert failure on mismatched res/depth ranges
        with pytest.raises(ValueError):
            mo = Model(station_locations=do.station_locations,
                       cell_size_east=500,
                       cell_size_north=500,
                       ...
                       z_target_depth=120000,
                       res_initial_method='by_range',
                       res_initial_custom_values=[200, 300],
                       res_initial_custom_depth_ranges=[(0, 1000)])
            mo.make_mesh()
        ...
        mo = Model(station_locations=do.station_locations,
                   ...
                   res_initial_method='by_range',
                   res_initial_custom_values=[200, 300],
                   res_initial_custom_depth_ranges=[(0, 1000), (5000, 10000)])

        mo.make_mesh()
        mo.write_model_file(save_path=self._output_dir)

        for test_output, expected_output in (
                ("ModEM_Data.dat", "ModEM_Data.dat"),
                ("ModEM_Model_File.rho", "ModEM_Model_File.rho")
        ):
            output_data_file = os.path.normpath(os.path.join(self._output_dir, test_output))
