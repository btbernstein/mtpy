...
from mtpy.utils import modem_utils as mu


def test_strip_padding():
    grid_axis = [-2., -1., 0., 1., 2.]
    test = mu.strip_padding(grid_axis, 1)
    expected = [-1., 0., 1.]
    np.testing.assert_array_equal(test, expected)

    # For 0 padding case, the array should come back untouched.
    test = mu.strip_padding(grid_axis, 0)
    expected = grid_axis
    np.testing.assert_array_equal(test, expected)
    ...
    test = mu.strip_padding(test, 1, keep_start=True)
    expected = [-2., -1., 0., 1.]
    np.testing.assert_array_equal(test, expected)
...
def test_gdal_origin():
    ...
    center_point = 20., 150.
    test = mu.get_gdal_origin(grid_east, 1., center_point[1],
...
def test_strip_resgrid():
    ...
    resgrid = resgrid.T
    test = mu.strip_resgrid(resgrid, 1, 1, 1)
...
def test_get_depth_indicies():
    ...
    assert mu.get_depth_indices(z_cells, [49]) == set([0])
    assert mu.get_depth_indices(z_cells, [500]) == set([2])

    # Should round up to nearest depth
    assert mu.get_depth_indices(z_cells, [50]) == set([1])

    # If two provided depths are closest to same index, only return
    #  that index (by returning a set)
    assert mu.get_depth_indices(z_cells, [50, 51]) == set([1])

    # If no depths are provided, return every index in a set
    assert mu.get_depth_indices(z_cells, []) == {0, 1, 2, 3, 4}
